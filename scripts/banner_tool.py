#!/usr/bin/env python3
"""banner_tool.py — Self-contained banner generator.

ONE entry point for the engine, the post subagent, the CLI, and tests.
Reads tokens from assets/brand-config.json → banner, fills CSS variables
in scripts/banner-templates/{name}.html, renders the resulting HTML to
...
def load_tokens(vault: Path | None = None) -> dict:
    """Read banner config from brand-config.json. Validates required keys."""
    vault = vault or _vault()
    cfg_path = vault / "assets" / BRAND_CONFIG
    if not cfg_path.exists():
        raise FileNotFoundError(f"brand-config.json not found at {cfg_path}")
    cfg = json.loads(cfg_path.read_text())

    if "banner" not in cfg:
        raise ValueError(
            f"brand-config.json missing 'banner' section. "
            f"Run migration: see banner_tool README."
        )

    banner = cfg["banner"]
    tokens = dict(banner.get("tokens", {}))
    missing = REQUIRED_TOKENS - set(tokens)
    if missing:
        raise ValueError(
            f"banner.tokens missing required keys: {sorted(missing)}. "
            f"Add them to assets/brand-config.json under banner.tokens."
        )
    ...
    tokens["_template"] = banner.get("template", "default")
...
def load_template(vault: Path | None = None, name: str = "default") -> str:
    ...
    vault = vault or _vault()
    path = vault / "scripts" / TEMPLATES_DIR / f"{name}.html"
    if not path.exists():
        raise FileNotFoundError(
            f"Banner template not found: {path}\n"
            f"Create scripts/{TEMPLATES_DIR}/{name}.html "
            f"(see scripts/banner-templates/default.html for the contract)."
        )
    html = path.read_text()
...
def build_html(
    ...
    # Pick + load icon SVG.
    icon_name = pick_icon(tokens, title, subtitle)
    ...
    title_lines_html = "\n    ".join(
        f'<div class="title-line">{line}</div>' for line in title_lines
    )
    subtitle_html = (
        f'<div class="subtitle-wrapper"><span class="subtitle-highlight">{subtitle}</span></div>'
        if subtitle
        else ""
    )
    ...
    html = html.replace("__SUBTITLE_HTML__", subtitle_html)
    html = html.replace("__HANDLE_TEXT__", handle)
...
def render_png(
    ...
    try:
        with sync_playwright() as p:
            ...
            # Give web fonts a brief chance to load (offline → fallback).
            try:
...
def generate(
    ...
    if not template or template == "default":
        template = tokens.get("_template", "default")
    tmpl = load_template(vault, template)
    if not handle:
        cfg = json.loads((vault / "assets" / BRAND_CONFIG).read_text())
...
def generate_for_draft(vault: Path | None, draft_path: Path) -> Path | None:
...
def generate_for_queue(vault: Path | None = None) -> list[Path]:
    ...
    for draft in drafts:
        print(f"  {draft.stem}")
        result = generate_for_draft(vault, draft)
        if result:
            out.append(result)
    print(f"\n  Generated: {len(out)}/{len(drafts)} banners")
    return out
...
def _cli() -> int:
    ...
    sub.add_parser("generate-queue", help="Render banners for every content/queue/*.md")
    ...
    if args.cmd == "generate-draft":
        out = generate_for_draft(vault, Path(args.file))
        return 0 if out else 1
    if args.cmd == "test":
