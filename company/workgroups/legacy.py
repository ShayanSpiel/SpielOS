...
def workgroup_from_legacy(department: Department) -> WorkgroupSpec:
    ...
    for worker_id in tuple(getattr(department, "agent_ids", ()) or ()):
        ...
        if agent is None:
            # Package validation will identify an undeclared worker. Preserve it
            # here so catalog diagnostics remain useful during migration.
...
class WorkgroupHandler(InterpretedDepartment, Department):
    ...
    This class is not a Workgroup domain object.  It exists only because the
    durable loop currently indexes goal owners by a GoalHandler protocol.
    """
    ...
    def __init__(self, workgroup: WorkgroupSpec, legacy: Department):
        ...
        self.goal_schema = {
            "metrics": list(workgroup.metrics),
    ...
    def __getattr__(self, name):
        ...
        Content machine gates and the Outbound email exception are implementation
        details of their Workers' workkits, not Workgroup behavior.  Delegation
        keeps those hooks available without making the Workgroup their owner.
        """
