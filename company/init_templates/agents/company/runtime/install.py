...
def synthesize_agents(normalized: dict[str, Any],
    ...
    for item in explicit or []:
        ...
        by_id[agent_id] = {
            "id": agent_id,
            "description": str(item.get("description") or f"{agent_id} Agent"),
    ...
    for agent_id in normalized["agent_ids"]:
        ...
        for workflow in normalized["workflows"]:
            for node in workflow.get("graph") or []:
                if (node.get("agent_id") or node.get("employee_id")) == agent_id:
...
def normalize_department_spec(raw: dict[str, Any], *,
    ...
    for workflow in workflows:
        ...
        for node in workflow["graph"]:
            if node.get("kind") == "employee":  # retired package input
                node["kind"] = "agent"
            ...
            node["agent_id"] = node.get("agent_id") or node.pop("employee_id", None)
            if node["kind"] == "agent" and not node.get("agent_id"):
                node["agent_id"] = workflow_agents.get(workflow["id"]) or agent_ids[0]
            if node["kind"] == "agent" and not node["produces"]:
                node["produces"] = list(
                    workflow.get("evidence") or evidence_metrics.get(metrics[0], ["artifact"]))
            if node.get("agent_id") and node["agent_id"] not in agent_ids:
...
def validate_department_spec(spec: dict[str, Any]) -> list[str]:
    ...
    for workflow in normalized["workflows"]:
        ...
        for node in workflow.get("graph") or []:
            ...
            kind = node.get("kind")
            if kind not in {"agent", "approval", "connection", "machine"}:
                defects.append(f"workflow {workflow_id} unknown step kind {kind}")
            if kind == "agent" and not node.get("agent_id"):
                defects.append(f"workflow {workflow_id} Agent step needs agent_id")
            if node.get("agent_id") and node["agent_id"] not in known_agents:
                defects.append(
...
def _py_step(node: dict[str, Any]) -> str:
    return (
        f"WorkflowStep({node.get('id')!r}, {node.get('kind', 'agent')!r}, "
        f"{node.get('agent_id')!r}, produces={_py_tuple(node.get('produces') or [])}, "
...
def install_agents(agents: list[dict[str, Any]], *,
    ...
    for agent in agents:
        ...
        if agent_id in AGENTS:
            # Packages may reuse a built-in Agent but never shadow it.
            continue
        stem = agent_file_stem(agent_id)
        ...
        payload = {
            "id": agent_id,
            "description": str(agent.get("description") or f"{agent_id} Agent"),
...
def _install_live_transaction(normalized: dict[str, Any], *, force: bool,
    ...
    for agent_id in normalized["agent_ids"]:
        ...
        if existing_id and (existing_id != agent_id or agent_id not in target_owned):
            raise ValueError(
                f"Agent '{agent_id}' conflicts with installed Agent '{existing_id}'")
    installable_agents = [
