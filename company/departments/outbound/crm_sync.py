...
in the OpenCode host (``company/connections/registry.py``) and is
...
def project_root() -> Path:
    """Repo root — `company/departments/outbound/` parents[3] is
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--plan", default=str(
        Path(__file__).resolve().parents[3] / ".spielos" / "state" / "outbound" / "crm_sync_plan.json"),
