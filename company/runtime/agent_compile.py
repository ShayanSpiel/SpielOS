"""Compile a department workflow into a FIRST-CLASS AGENT WORKER.
...
The compiled worker is scoped to exactly that department + workflow: it may
...
def _render_opencode(root: Path, name: str, department_id: str, workflow_id: str,
    ...
    path.write_text(f"""---
description: First-class {name} worker ({department_id}/{workflow_id}). Runs only this department's workflow.
mode: primary
temperature: 0.2
permissions:
  edit: deny
...
workflow inside the `{department_id}` Department of this SpielOS harness.
...
def _render_codex(root: Path, name: str, department_id: str, workflow_id: str,
    ...
    path.write_text(f"""# First-class {name} worker — scope: {department_id}/{workflow_id}
# Generated by `spielos agent compile`. Do not hand-edit scope fields.
name = "{name}"
description = "First-class executor of the {workflow_id} workflow in the {department_id} Department. Not the Director; runs only this workflow."
...
You operate as {name}: execute the {workflow_id} workflow of the {department_id}
Department only. Follow the company runtime CLI contract (company tasks /
work orders), record honest evidence, park live external actions for approval,
and never touch other departments' goals or files.
""")
    return str(path)
