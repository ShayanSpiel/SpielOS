...
TEMPLATE_ROOT = COMPANY_ROOT / "departments/design/templates"
...
def render_context_tokens():
    ...
    names = set()
    for css in (
        COMPANY_ROOT / "departments/design/system/production.css",
...
class CatalogTests(unittest.TestCase):
    ...
    def test_lean_layout_has_one_authority_and_no_duplicate_layers(self):
        self.assertTrue((COMPANY_ROOT / "README.md").is_file())
        self.assertTrue((COMPANY_ROOT / "strategy/icp.md").is_file())
        self.assertFalse((COMPANY_ROOT / "engines").exists())
        self.assertFalse((COMPANY_ROOT / "tools").exists())
        self.assertFalse((COMPANY_ROOT / "workgroups").exists())
    ...
    def test_canonical_video_templates_stay_with_design(self):
        root = COMPANY_ROOT / "departments/design/templates/video"
...
class TemplateRegistryTests(unittest.TestCase):
    ...
    def test_selection_rule_never_repeats_a_template_in_a_batch(self):
        ...
        readme = (COMPANY_ROOT / "departments/design/README.md").read_text()
        self.assertIn("no batch repeats one template", readme.lower())
        skill = (COMPANY_ROOT / "skills/video-creation/SKILL.md").read_text()
...
class SocialWorkflowTests(unittest.TestCase):
    ...
    def test_outbound_social_goal_requests_agent_then_evaluates_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("social-researcher", open_orders[0]["agent_id"])
