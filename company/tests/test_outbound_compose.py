...
class SubjectBankTests(unittest.TestCase):
    def test_portable_default_bank_is_available_without_private_state(self):
        bank = content.subject_bank_for("generic-workflow")
        self.assertGreaterEqual(len(bank), 3)
        self.assertTrue(all("{company}" in subject for subject in bank))
        self.assertEqual(bank, content.subject_bank_for("unknown-segment"))
