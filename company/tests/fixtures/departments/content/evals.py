"""Content eval registry. Criteria are identifiers with source references;
rules live in text authorities (migrated 2026-09-03)."""
...
def _criterion(identifier: str, name: str, source: str) -> EvalCriterion:
    return EvalCriterion(id=identifier, name=name,
                         description=f"Source: {source}", source=source)
...
CONTENT_COPY_TOP10 = EvalSuite(
    ...
    payload_id_selector=lambda payload: str(
        payload.get("batch_id") or payload.get("id") or "payload"),
