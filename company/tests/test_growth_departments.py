...
class GrowthDepartmentTests(unittest.TestCase):
    def test_all_bundled_departments_are_discovered_from_their_own_folders(self):
    ...
    def test_representative_departments_share_frozen_lego_contract(self):
        ...
        step_fields = {
            "id", "kind", "agent_id", "produces", "requires", "skill_ids",
            "connection_ids",
        }
        for department in representatives.values():
            ...
            for workflow in department["workflows"]:
                ...
                for step in workflow["graph"]:
                    ...
                    self.assertIn(step["kind"], {"agent", "approval", "connection", "machine"})
    ...
    def test_content_publish_is_approval_gated_then_uses_direct_buffer_connection(self):
        ...
        request = result.payload["connection_request"]
        self.assertEqual("buffer", request["connection_id"])
        self.assertEqual("dry_run", request["execution_mode"])
...
class DesignContractTests(unittest.TestCase):
    ...
    def test_campaign_video_templates_are_restored_batch1_flat_with_still_thumbnail_titles(self):
        ...
        sources = "\n".join((root / path).read_text() for path in (
            "departments/design/templates/video/scenario-b.html",
            "departments/design/templates/video/scenario-c.html",
        ))
