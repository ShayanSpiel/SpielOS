...
class _MemoryDepartment(InterpretedDepartment, Department):
    ...
    workflows = (WorkflowSpec(
        ...
        ("researcher",), (), (), ("output",), (), graph=(
            WorkflowStep("produce", "agent", agent_id="researcher",
                         produces=("output",)),
        )), WorkflowSpec(
        "secondary", "A distinct workflow outside the learned scope", ("produce",),
        ("researcher",), (), (), ("output",), (), graph=(
            WorkflowStep("produce", "agent", agent_id="researcher",
                         produces=("output",)),
        )))
...
class MemorySemanticsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root = self.runtime.create_goal(
            name="Memory behavior test root", owner_id="memory_test",
            metric="outputs", operator="ge", target=100,
